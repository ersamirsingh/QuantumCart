...
// //             <span className="som-items-badge">{order.items.length} items</span>
// //          </div>
...
// //                {/* Order Items */}
// //                <div className="som-detail-section">
// //                   <div className="som-detail-label">Order Items ({order.items.length})</div>
...
//                         </div>
//                      )}
//                      {order.shippingAddress?.postalCode && <div>PIN: {order.shippingAddress.postalCode}</div>}
...
export default function SellerOrderManagement() {
   ...
   const handleShipOrder = async (orderId, weight) => {
      try {
         await axiosClient.post("/shipment/ship", { orderId, orderWeight: weight });
         fetchOrders();
         alert("Order shipped successfully!");
      } catch (error) {
         alert(`Failed to ship order: ${error.response?.data?.message || error.message}`);
